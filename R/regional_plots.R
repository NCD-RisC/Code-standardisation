...
regional_plot_year <- function(df, group_var, age_low = NULL, age_high = NULL,
    ...
    for(j in 1:length(sregions)) {
        ...
        for(i in 1:length(age_low)) {
            ...
            plot(dat_plot$year_var,dat_plot[, 'mean_var'],pch=scope.pch,col=coverage_col,
                 xlim = my_xlim, 
                 ylim = my_ylim,
                 cex=.6,main=paste(sregions[j],"\n",
                                   age_low[i]," <= year < ",age_high[i],sep=''),xlab='',ylab='', cex.main=.8)
            
            for(i in 1:nrow(dat_plot)) {
                lines(c(dat_plot$year_var[i],dat_plot$year_var[i]),c((dat_plot[, 'mean_var'][i] - 2*dat_plot[, 'se_var'][i]),
                                                                     (dat_plot[, 'mean_var'][i] + 2*dat_plot[, 'se_var'][i]) ),col=coverage_col[i])
            }
        }
    }
...
regional_plot_age <- function(df, group_var, year_low = NULL, year_high = NULL, my_xlim = c(5, 19), 
    ...
    for(j in 1:length(sregions)) {
        ...
        for(i in 1:length(year_low)) {
            ...
            plot(dat_plot$age,dat_plot[, 'mean_var'],pch=scope.pch,col=coverage_col,
                 xlim = my_xlim, 
                 ylim = my_ylim,
                 cex=.6,main=paste(sregions[j],"\n",
                                   year_low[i]," <= year < ",year_high[i],sep=''),xlab='',ylab='', cex.main=.8)
            
            for(i in 1:nrow(dat_plot)) {
                lines(c(dat_plot$age[i],dat_plot$age[i]),c((dat_plot[, 'mean_var'][i] - 2*dat_plot[, 'se_var'][i]),
                                                           (dat_plot[, 'mean_var'][i] + 2*dat_plot[, 'se_var'][i]) ),col=coverage_col[i])
            }
        }
    }
