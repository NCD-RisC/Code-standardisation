...
#Adapted by BZ for dual likelihood 3/7/23
...
# AR May 2024 includes initial values 
...
mod.no              <- 242                                                              # Set a unique model identifier: this can be any number or a character string
...
covariate.file.name <- "/mnt/covariates-urbanisation-2021.csv"                               # Set covariate filename
data.file.name      <- "/mnt/model_ready_glucose_data_WITHOUT_new_CCDRFS-v2.csv"                                   # Set data filename
country.file.name   <- "/mnt/country-list-2023-new.csv"
initial.vals.file.name <- paste0("Model",last.mod.no, "_",name_var,"_",sex,"_Seed_",seedVal,"_initial_values_windices.RData")


outdirname <- paste0("/mnt/", name_var, "_", sex, "/")                                           # Set up output directory
...
source("/mnt/1_functions.R")                            # Loads functions
...
data                    <- data[which(data[,model.indicator] == 1),]
...
data$cwvar_probit[is.na(data$cwvar_probit)] <- 0
...
phi_c[1]                <- initial_values[index_list[["phi_c"]]] 
phi_r[1]                <- initial_values[index_list[["phi_r"]]]  
phi_s[1]                <- initial_values[index_list[["phi_s"]]]  
eta_c[1]                <- initial_values[index_list[["eta_c"]]]
eta_r[1]                <- initial_values[index_list[["eta_r"]]]
eta_s[1]                <- initial_values[index_list[["eta_s"]]]      
                        
                        

phi_natl[1]             <- initial_values[index_list[["phi_natl"]]]         
phi_subn[1]             <- log(exp(initial_values[index_list[["phi_natl"]]]) + exp(initial_values[index_list[["phi_diff_subn_natl"]]])) 
phi_comm[1]             <- log(exp(initial_values[index_list[["phi_natl"]]]) + exp(initial_values[index_list[["phi_diff_subn_natl"]]])  + exp(initial_values[index_list[["phi_diff_comm_subn"]]]))  

tau[1]                  <- initial_values[index_list[["tau"]]]         

theta.max        <- 20
theta.max.prime  <- 1/(exp(theta.max))
theta_g[1]       <- log(1/(theta.max.prime + exp(initial_values[index_list[["theta_diff_g_max"]]]))) 
...
theta_c[1]       <- log(1/(theta.max.prime + exp(initial_values[index_list[["theta_diff_g_max"]]]) + 
                                             exp(initial_values[index_list[["theta_diff_s_g"]]]) + 
                                             exp(initial_values[index_list[["theta_diff_r_s"]]]) + 
                                             exp(initial_values[index_list[["theta_diff_c_r"]]]))) 

gamma[1,]               <- initial_values[index_list[["gamma"]]]  

sigma1_c[1,]            <- exp(initial_values[index_list[["sigma1_c"]]])              
sigma2_c[1,]            <- exp(initial_values[index_list[["sigma2_c"]]])              
sigma1_r[1,]            <- exp(initial_values[index_list[["sigma1_r"]]])             
sigma2_r[1,]            <- exp(initial_values[index_list[["sigma2_r"]]])              
sigma1_s[1,]            <- exp(initial_values[index_list[["sigma1_s"]]])              
sigma2_s[1,]            <- exp(initial_values[index_list[["sigma2_s"]]])              

theta[1,1:length(index_list[["theta_1"]])]                      <- initial_values[index_list[["theta_1"]]]          
theta[1,(length(index_list[["theta_1"]]) + N + 1):dim(F)[2]]    <- initial_values[index_list[["theta_2"]]]          

u[1,]                   <- initial_values[index_list[["u"]]]                
v[1,]                   <- initial_values[index_list[["v"]]]                 
sv[1,]                  <- initial_values[index_list[["sv"]]]                 
w[1,]                   <- initial_values[index_list[["w"]]]                  

###sample alpha and zeta later
...
#sample non modelled latent variable from IV
zeta[1,]                <- mvrnorm(n = 1, mu = rep(mean(initial_values[index_list[["zeta"]]]), I), Sigma = diag(rep(var(initial_values[index_list[["zeta"]]]),I)))
# Sample starting values for alpha from likelihood conditional on other parameter values
